import globalStyle from "@/outter/fr-schema-antd-utils/src/components/GGeditor/components/Flow/common/globalStyle"
import FSM, {
    StateTable
} from "@/outter/fr-schema-antd-utils/src/components/GGeditor/components/Flow/common/stateMachine"
import { NodeModel } from "@/outter/fr-schema-antd-utils/src/components/GGeditor/common/interfaces"
...
function handleAnchor(this: ThisProps, name: string, value: any, item: Item) {
    ...
    const anchors = group
        .get("children")
        .filter(e =>
            ["anchor", "anchorSpot", "banAnchor"].some(
                a => e.get("className") === a
            )
        )
    ...
    const activeAnchor = () =>
        anchors.forEach(a => {
            ...
            if (item.hasState("addingEdge"))
                a.setHotspotActived && a.setHotspotActived(true)
            else a.setActived()
        })
    ...
    const clearActivedAnchor = () =>
        anchors.forEach(a => {
            if (a.get("index") != name.slice(-1)) return
            if (item.hasState("addingEdge"))
                a.setHotspotActived && a.setHotspotActived(false)
            else a.clearActived()
        })
    ...
    const removeAllAnchorSpot = () =>
        !item.hasState("addingEdge") && anchors.forEach(a => a.remove())
    ...
    if (isActiveAnchor(name))
        value ? fsm.transtion("enterAnchor") : fsm.transtion("leaveAnchor")
