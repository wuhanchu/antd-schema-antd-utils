import { ShapeClassName, ItemState } from "@/outter/fr-schema-antd-utils/src/components/GGeditor/common/constants"
import { CustomShape, NodeModel } from "@/outter/fr-schema-antd-utils/src/components/GGeditor/common/interfaces"
...
const options: CustomShape<G6.Node, NodeModel> & {
        ...
        if (model.statusIconColor) {
            ...
            group.addShape("circle", {
                ...
                attrs: {
                    ...
                    y: 7,
                    fill:
                        typeof model.statusIconColor === "string"
                            ? model.statusIconColor
                            : defaultColor
                }
            })
        }
    ...
    resetLabelText(label: G.Shape, maxWidth: number, maxLine = 2): string {
        ...
        for (const char of lastLine) {
            if (
                UtilCanvasContext.measureText(newLastLine + char).width <
    ...
    setState(name, value, item) {
        ...
        const wrapper = item
            .getContainer()
            .findByClassName(ShapeClassName.Wrapper)
